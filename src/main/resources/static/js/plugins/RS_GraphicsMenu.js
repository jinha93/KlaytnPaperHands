...
(function () {
  ...
  RS.GraphicsMenu.Params.goldImage = parameters["Gold Image"];
  ...
  // Window_Gold
  //============================================================================
  class Window_RSGraphicsMenuGold extends Window_Gold {
    constructor() {
      super(0, 0);
     ...
     */
    createGoldSprite() {
      const texture = PIXI.Texture.from(`img/pictures/${RS.GraphicsMenu.Params.goldImage}.png`);
      this._gold = new PIXI.Sprite(texture);
      this._gold.anchor.set(0.0, 0.0);
      
      this._windowContentsSprite.addChild(this._gold);
    }
    ...
    drawGoldIcon(x, y) {   
      ...
      this._gold.x = this.textPadding();
      this._gold.y = this.textPadding();
      
      return this._gold.width + 2;
    ...
    refresh() {
      ...
      var width = this.contents.width - x - this.textPadding() * 2;
  ...
  Scene_LinearMenu.prototype.create = function () {
    ...
    // YEP_CommonEventMenu 플러그인과 호환성을 맞춥니다.
  ...
  Scene_LinearMenu.prototype.createGoldWindow = function() {
    this._goldWindow = new Window_RSGraphicsMenuGold();
  ...
  Scene_LinearMenu.prototype.updateIndex = function () {
    ...
    // 오른쪽 방향키를 눌렀는가?
    if(Input.isTriggered('right')) this.right();
    // 왼쪽 방향키를 눌렀는가?
    if(Input.isTriggered('left')) this.left();
    // 결정키를 눌렀는가?
    if(Input.isTriggered('ok')) this.selectScene();

    // 이 함수는 마우스 포인터 또는 손가락으로 메뉴를 고를 수 있게 합니다.
  ...
  Scene_LinearMenu.prototype.selectScene = function () {
    ...
    if(sceneObject.match(/(?:EVAL[ ]*:[ ]*)(.*)/i)) {
      try {
        this._touched = false;
        eval(RegExp.$1); // 스크립트 구문 평가 및 호출
        SoundManager.playOk();
      } catch(e) {
        console.warn(e);
      }
    }
    // 유효한 씬 이름일 때, 해당 씬을 호출합니다.
